...
import { SchematicStore, ensureInBounds, ensureRegionInBounds, regionVolume, SUPPORTED_MC_VERSIONS, } from './store.js';
...
const SUPABASE_KEY = process.env.MCMCP_SUPABASE_ANON_KEY ?? 'sb_publishable_xUIUglzmwDQc91YAZXhXJQ_BZeMCaS1';
...
let activeSession = ENV_SESSION;
const server = new McpServer({ name: 'mcmcp-schematic', version: '0.1.0' }, {
    instructions: 'Build Minecraft schematics block by block. Always call create_zone first (or pass an existing session_id from the viewer). ' +
...
const resolveSession = (provided) => {
    const id = provided ?? activeSession;
    if (!id) {
        throw new Error('No active session. Call use_session({ session_id }) first, or pass session_id as a tool argument, or set MCMCP_SESSION_ID in the MCP env.');
    }
    return id.toUpperCase();
};
server.registerTool('use_session', {
    title: 'Switch the active session',
    description: 'Set the active session_id used by every subsequent tool call (until changed again). ' +
    ...
    try {
        ...
        activeSession = id;
        return text(`Active session is now ${id} (${session.size_x}×${session.size_y}×${session.size_z}, mc=${session.mc_version}). ` +
...
server.registerTool('current_session', {
    ...
    try {
        ...
        return text(`session=${activeSession} size=${session.size_x}×${session.size_y}×${session.size_z} mc=${session.mc_version} blocks=${rows.length}\n` +
...
server.registerTool('set_version', {
    title: 'Change the Minecraft version of the active session',
    description: `Update the active session's Minecraft version. The viewer reloads its texture pack to match. ` +
...
server.registerTool('create_zone', {
    ...
    description: 'Creates a new schematic session of the given dimensions, or resets the existing session_id. ' +
    ...
    try {
        ...
        return text(`Zone ready and now active. session_id=${session.id} size=${session.size_x}×${session.size_y}×${session.size_z} mc=${session.mc_version}\n` +
...
server.registerTool('set_block', {
    ...
    description: 'Place one block at (x,y,z). Use block="air" to clear. Coordinates must be inside the zone. Slow for large patterns — use set_blocks instead.',
    inputSchema: {
    ...
    try {
        ...
        if (validErr)
            return error(validErr);
        const id = resolveSession(session_id);
        const session = await store.getSession(id);
        const inputs = [{ x, y, z, block }];
...
server.registerTool('set_blocks', {
    title: 'Set or clear a batch of blocks',
    description: 'Place or clear many blocks at once (e.g. a whole layer). All coordinates must be inside the zone — the call fails atomically if any are out of bounds. block="air" clears that position.',
    inputSchema: {
        blocks: z
            .array(z.object({
            x: z.number().int(),
    ...
    try {
        ...
        const id = resolveSession(session_id);
        const session = await store.getSession(id);
        ensureInBounds(session, blocks);
        const { written, cleared } = await store.setBlocks(id, blocks);
...
server.registerTool('fill_region', {
    title: 'Fill a cuboid with one block type',
    description: 'Fill every position in the inclusive cuboid (x1,y1,z1)..(x2,y2,z2) with the given block. ' +
    ...
    try {
        ...
        const id = resolveSession(session_id);
        const session = await store.getSession(id);
        const region = { x1, y1, z1, x2, y2, z2 };
        ...
        return text(`Filled (${norm.x1},${norm.y1},${norm.z1})..(${norm.x2},${norm.y2},${norm.z2}) ` +
...
server.registerTool('fill_layer', {
    title: 'Fill an entire layer with one block type',
    description: 'Fill a whole plane of the zone. axis="y" (a horizontal floor/ceiling), "x" (a wall east-west), or "z" (a wall north-south). ' +
...
server.registerTool('replace', {
    ...
    description: 'Swap every from_block with to_block in the session, optionally restricted to a cuboid region. ' +
...
server.registerTool('get_region', {
    ...
    description: 'Return a dense 3D array for the inclusive cuboid (x1,y1,z1)..(x2,y2,z2). ' +
    ...
    try {
        ...
        const dense = Array.from({ length: sy }, () => Array.from({ length: sz }, () => Array(sx).fill('air')));
        for (const r of rows) {
            dense[r.y - norm.y1][r.z - norm.z1][r.x - norm.x1] = r.block_type;
        }
        const header = `region=(${norm.x1},${norm.y1},${norm.z1})..(${norm.x2},${norm.y2},${norm.z2}) ` +
...
server.registerTool('get_layer', {
    ...
    description: 'Return a dense 2D array for the plane axis=index. Array shape depends on axis: ' +
    ...
    try {
        ...
        if (axis === 'y') {
            ...
            dimA = session.size_z;
            dimB = session.size_x;
            dense = Array.from({ length: dimA }, () => Array(dimB).fill('air'));
            for (const r of rows) {
...
server.registerTool('get_all', {
    title: 'Read all blocks in the session',
    description: 'Return every placed block with its coordinates and type, plus the zone size. Use this to inspect the current state.',
    inputSchema: {
        session_id: z.string().length(6).optional(),
    },
}, async ({ session_id }) => {
...
server.registerTool('export_litematic', {
    title: 'Export the session as a .litematic file',
    description: 'Encode the current session as a Litematica schematic (.litematic) and write it to disk. ' +
    ...
    try {
        ...
        return text(`Exported ${result.totalBlocks} blocks (palette=${result.paletteSize}, bits/entry=${result.bitsPerEntry}, ` +
