...
function parseBlockState(raw) {
    const trimmed = raw.trim();
    const m = trimmed.match(RE_STATE);
    if (!m)
        throw new Error(`Invalid block id "${raw}"`);
    const head = m[1];
...
function bitsPerEntryFor(paletteSize) {
 ...
 */
function packBlockStates(indices, bitsPerEntry) {
...
const intTag = (v) => ({ type: 'int', value: v });
const stringTag = (v) => ({ type: 'string', value: v });
const longTag = (v) => {
    const hi = Number((v >> 32n) & 0xffffffffn) | 0;
    const lo = Number(v & 0xffffffffn) | 0;
    return { type: 'long', value: [hi, lo] };
};
const xyzCompound = (x, y, z) => ({
    type: 'compound',
    value: { x: intTag(x), y: intTag(y), z: intTag(z) },
});
export function buildLitematic(session, rows, opts = {}) {
    ...
    const emptyCompoundList = {
        type: 'list',
        value: { type: 'compound', value: [] },
    };
    const blockStatesArray = { type: 'longArray', value: packed };
    const region = {
        type: 'compound',
        value: {
