...
export const SUPPORTED_MC_VERSIONS = ['1.20', '1.21'];
export function isMcVersion(v) {
    return SUPPORTED_MC_VERSIONS.includes(v);
}
...
function randomSessionId(length = 6) {
    let out = '';
...
export class SchematicStore {
    ...
    async createSession(size_x, size_y, size_z, preferredId, mcVersion) {
        ...
        if (preferredId) {
            ...
            if (existing) {
                ...
                if (clearErr)
                    throw new Error(`Failed to clear session: ${clearErr.message}`);
    ...
    async setSessionVersion(id, mcVersion) {
        const { data, error } = await this.sb
            .from('mcmcp_sessions')
            ...
            .single();
        if (error || !data)
            throw new Error(`Failed to update session version: ${error?.message}`);
        const session = data;
    ...
    async getSession(id) {
        ...
        if (!data)
            throw new Error(`Session ${id} does not exist. Open the viewer to create one, or call create_zone first.`);
        const session = data;
        this.sessionCache.set(id, { session, cachedAt: Date.now() });
        return session;
    }
    ...
    async setBlocks(sessionId, blocks) {
        if (blocks.length === 0)
            return { written: 0, cleared: 0 };
        const air = [];
        const solid = [];
        for (const b of blocks) {
            if (normalizeBlockType(b.block) === 'air')
                air.push(b);
            else
                solid.push(b);
        }
        ...
        if (air.length > 0) {
            ...
            const { error, count } = await this.sb
                ...
                .or(ors);
            if (error)
                throw new Error(`Failed to delete air blocks: ${error.message}`);
            cleared = count ?? 0;
        }
        let written = 0;
        if (solid.length > 0) {
    ...
    async fillRegion(sessionId, region, block) {
        ...
        if (type === 'air') {
            ...
            if (error)
                throw new Error(`Failed to clear region: ${error.message}`);
    ...
    async getRegion(sessionId, region) {
        const r = normalizeRegion(region);
        const all = [];
        const PAGE = 1000;
        let from = 0;
    ...
    async replaceBlocks(sessionId, fromBlock, toBlock, region) {
        ...
        if (toType === 'air') {
            ...
            if (error)
                throw new Error(`Failed to clear ${fromType}: ${error.message}`);
        ...
        if (error)
            throw new Error(`Failed to replace ${fromType}: ${error.message}`);
...
export function ensureRegionInBounds(session, region) {
    const r = normalizeRegion(region);
    const out = (r.x1 < 0 || r.x2 >= session.size_x ||
        r.y1 < 0 || r.y2 >= session.size_y ||
        r.z1 < 0 || r.z2 >= session.size_z);
    if (out) {
        throw new Error(`Region (${r.x1},${r.y1},${r.z1})..(${r.x2},${r.y2},${r.z2}) exceeds zone ` +
            `${session.size_x}×${session.size_y}×${session.size_z}. ` +
            `Valid range: x∈[0,${session.size_x - 1}], y∈[0,${session.size_y - 1}], z∈[0,${session.size_z - 1}].`);
    }
    return r;
}
export function ensureInBounds(session, blocks) {
    const out = [];
    for (const b of blocks) {
        if (b.x < 0 || b.x >= session.size_x ||
    ...
    if (out.length > 0) {
        ...
        throw new Error(`Out of bounds: ${out.length === 5 ? 'at least 5' : out.length} block(s) outside zone ` +
