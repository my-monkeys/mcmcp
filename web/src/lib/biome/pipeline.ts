...
/**
 * Carve serpentine river channels using a separate noise stream. Cells where
 * |riverNoise(x*freq, z*freq)| < threshold get their surface block replaced
 * with water; the block one below becomes dirt (river bed). Produces thin
 * meandering bands rather than blob-shaped lakes.
 */
function riverPass(
  cfg: BiomeConfig,
  cells: Map<Cell, string>,
